...
- `command-routing-drift` — catches catalog, command, skill-list, artifact, and stop-condition mismatches before an agent follows the wrong route.
...
- `runtime-activation-bootstrap` — proves clean installed-plugin sessions route vague requests through the bootstrap gate before code edits.
- `session-start-hook-bootstrap` — proves session-start hooks inject the bootstrap before answers, questions, or edits.
...
- `source-branded-skill-name` — rejects skill names, frontmatter, headings, and catalogs that preserve source branding instead of neutral operator jobs.
...
3. Run the target command or skill against the user request in the case, and confirm the harness route names an existing command or skill route before scoring output.
4. Score with the rubric in `evals/rubrics/agent-brain-rubric.md`.
5. Record the evidence checked, a pass/fail decision, risks, open questions, and fresh validation proof.
6. Save useful outputs under `evals/expected-artifacts/` only when they become golden examples.
