...
User request: implement one approved plan slice. Selected command: `/brain-build`. Command file: `commands/brain-build.md`. Loaded skills: `plan-slicing` and `qa-evidence`. Skill file: `skills/plan-slicing/SKILL.md`. Artifact: write `templates/changed-artifact-plus-implementation-notes.md`. Verification: record failing behavior proof or validator-first proof, passing proof, risks, and fresh validation proof before the next recommended state. Stop condition: stop if no approved slice, failing proof, or passing validation exists. Next state: VERIFY.
