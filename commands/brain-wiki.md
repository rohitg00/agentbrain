...
User request: update durable project guidance from checked evidence. Selected command: `/brain-wiki`. Command file: `commands/brain-wiki.md`. Loaded skills: `wiki-maintenance`, `activity-recap`, and `evidence-research`. Skill file: `skills/wiki-maintenance/SKILL.md`. Artifact: write `templates/wiki-update.md`. Verification: preserve neutral copy, cite source-backed evidence, validate changed docs, and record fresh validation proof before handoff. Stop condition: stop if provenance, freshness, or durable value is missing. Next state: LEARN.
