...
User request: decide whether a requested agent workflow should exist. Selected command: `/brain-should-this-exist`. Command file: `commands/brain-should-this-exist.md`. Loaded skills: `problem-grill` and `market-grill`. Skill file: `skills/problem-grill/SKILL.md`. Artifact: write `templates/non-agent-alternative-review.md`. Verification: compare user evidence, non-agent alternatives, costs, risks, and fresh validation proof before continuing to `/brain-research` or stopping. Stop condition: stop if non-agent alternatives, user evidence, risk, or success criteria are missing. Next state: RESEARCH.
