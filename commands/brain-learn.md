...
User request: preserve a repeated failure as reusable guidance. Selected command: `/brain-learn`. Command file: `commands/brain-learn.md`. Loaded skills: `learning-capture`, `context-memory`, and `wiki-maintenance`. Skill file: `skills/learning-capture/SKILL.md`. Artifact: write `templates/learning-capture.md`. Verification: update the matching skill, validator, or memory guidance with neutral copy and fresh validation proof before handoff. Stop condition: stop if the lesson is temporary chatter, private data, or not reusable. Next state: LEARN.
