...
User request: review changed artifacts before trust or release. Selected command: `/brain-review`. Command file: `commands/brain-review.md`. Loaded skills: `agent-output-verifier` and `engineering-grill`. Skill file: `skills/agent-output-verifier/SKILL.md`. Artifact: write `templates/review-report.md`. Verification: check correctness, security, maintainability, proof gaps, risks, and fresh validation proof before routing blockers to `/brain-build` or verified work to `/brain-ship`. Stop condition: stop if diff, evidence, security, maintainability, or side-effect scope is unchecked. Next state: SHIP.
