...
def test_command_examples_must_name_every_loaded_skill(tmp_path: Path) -> None:
    ...
    command.write_text(
        command.read_text(encoding="utf-8").replace(
            "Loaded skills: `sample`, `activity-recap`, `agent-output-verifier`, `ci-recovery`, `context-memory`, `domain-language`, `evidence-research`, `qa-evidence`, and `runtime-smoke`.",
...
def test_command_catalog_rejects_duplicate_command_entries(tmp_path: Path) -> None:
    ...
    catalog.write_text(
        catalog.read_text(encoding="utf-8")
        + "- [`/brain-sample`](brain-sample.md) — State: INTAKE; Use when: sample requests; Skills: `sample`, `activity-recap`, `agent-output-verifier`, `ci-recovery`, `context-memory`, `domain-language`, `evidence-research`, `qa-evidence`, `runtime-smoke`; Artifact: `templates/sample-routing-summary.md`; Stop: missing evidence.\n",
        encoding="utf-8",
    )
...
def test_readme_handoff_contract_must_include_resume_guidance(tmp_path):
    ...
    readme.write_text(
        readme.read_text(encoding="utf-8").replace(
            "Previous handoff notes are stale until checked; resume only the named next action after fresh validation proof confirms the current files, risks, and blockers.",
