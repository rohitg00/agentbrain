...
- **Exact command:** Full command or prompt invocation used for the smoke run. For `smoke_result: pass`, the recorded command must include the selected route (`--selected-command /brain-*`) and every loaded skill (`--loaded-skill <skill>`) so runtime routing evidence is reproducible instead of reconstructed after the fact.
