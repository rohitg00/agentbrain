...
The plugin bundle and wrappers expose `/brain-*` shortcuts while keeping `commands/brain-*.md` as the source of truth. The plugin bundle also carries an activation bootstrap, session-start hooks, command bodies, skills, templates, schemas, and core docs needed by plugin-based agents. Runtimes without proven plugin or custom slash-command support should use `AGENTS.md` and the command registry directly.
